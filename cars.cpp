...
const float dt = 0.01;
bool carCollision(float x_cars, float y_cars, float x_car, float y_car)
    {
        if (y_cars + 203 > y_car - 150 and y_cars + 100 < y_car + 150 and x_cars + 180 < x_car + 108 and x_cars + 180 > x_car - 70)
        {
            return true;
        } 
        return false;
...
int main()
{
    ...
    while (window.isOpen())
    {
        while (true)
        {
            ...
            for (int i = 0; i < 40; i++)
            {
                if (carCollision(x_cars[i], y_cars[i], x_car, y_car))
                {
                    coll = 1;
                    break;
                }    
